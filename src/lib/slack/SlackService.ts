...
export class SlackService {
  ...
  async summarizeThread(channelId: string, threadTs: string, userId: string) {
    ...
    try {
      ...
      // Get TL;DR and summary
      const { tldr, summary } = await this.aiService.summarizeThread(messages);
      console.log('Generated TL;DR and summary:', { tldr, summary });

      // Create message blocks with enhanced formatting
      const message = {
        blocks: [
          {
            type: "section",
        ...
        blocks: [
          {
            ...
            text: {
              type: "mrkdwn",
              text: "❌ Sorry, I couldn't generate a summary at this time. Please try again."
            }
          }
        ]
      };
      
      await this.postMessage(channelId, errorMessage);
      throw error;
    }
  }

  private async postMessage(channel: string, message: any) {
